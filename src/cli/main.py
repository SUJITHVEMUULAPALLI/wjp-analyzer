...
def command_opencv(args):
    """Convert image to DXF using OpenCV converter"""
    try:
        # Support both old relative path and the current package layout
        try:
        ...
        # Create converter with CLI parameters
        converter = OpenCVImageToDXFConverter(
            binary_threshold=args.threshold,
            min_area=args.min_area,
            dxf_size=args.dxf_size
        )
        
        # Generate output paths
        base_name = os.path.splitext(os.path.basename(args.image))[0]
        dxf_path = os.path.join(aout, f"{base_name}_opencv.dxf")
        preview_path = os.path.join(aout, f"{base_name}_opencv_preview.png")
        ...
        print(f"[green]OpenCV conversion complete[/green]")
        print(f"  Input: {args.image}")
        print(f"  DXF: {dxf_path}")
        print(f"  Preview: {preview_path}")
        print(f"  Contours: {result['contours_found']} found, {result['contours_kept']} kept")
        print(f"  Polygons: {result['polygons']} ({result['outer_polygons']} outer, {result['inner_polygons']} inner)")
        
    except Exception as e:
        print(f"[red]OpenCV conversion failed:[/red] {str(e)}")

def command_inkscape(args):
    """Convert image to DXF using Inkscape converter"""
    try:
        try:
            from wjp_analyser.image_processing.converters.inkscape_converter import (
                InkscapeImageToDXFConverter,
            )
        except Exception:
        ...
        # Generate output paths
        base_name = os.path.splitext(os.path.basename(args.image))[0]
        dxf_path = os.path.join(aout, f"{base_name}_inkscape.dxf")
        preview_path = os.path.join(aout, f"{base_name}_inkscape_preview.png")
        ...
        print(f"[green]Inkscape conversion complete[/green]")
        print(f"  Input: {args.image}")
        print(f"  DXF: {dxf_path}")
        print(f"  Preview: {preview_path}")
        print(f"  Trace Method: {result['trace_method']}")
        print(f"  Entities: {result['total_entities']}")
        print(f"  Entity Types: {result['entity_types']}")
        
    except Exception as e:
        print(f"[red]Inkscape conversion failed:[/red] {str(e)}")
        print(f"[yellow]Make sure Inkscape is installed and accessible via command line[/yellow]")
        print(f"[yellow]See INKSCAPE_GUIDE.md for installation instructions[/yellow]")
...
def main():
    ...
    # OpenCV converter command
    pocv = sub.add_parser("opencv", help="Convert image to DXF using OpenCV converter")
    pocv.add_argument("image")
    pocv.add_argument("--out", default="output")
    pocv.add_argument("--threshold", type=int, default=127, help="Binary threshold (0-255)")
    pocv.add_argument("--min-area", type=int, default=100, help="Minimum contour area")
    pocv.add_argument("--dxf-size", type=float, default=1000.0, help="DXF canvas size in mm")
    pocv.set_defaults(func=command_opencv)

    # Inkscape converter command
    pinsc = sub.add_parser("inkscape", help="Convert image to DXF using Inkscape (recommended)")
    pinsc.add_argument("image")
    pinsc.add_argument("--out", default="output")
    pinsc.add_argument("--trace-method", default="autotrace", choices=["autotrace", "potrace", "centerline"], help="Vectorization method")
    pinsc.add_argument("--threshold", type=float, default=0.5, help="Edge detection threshold (0.0-1.0)")
    pinsc.add_argument("--simplify", type=float, default=0.1, help="Path simplification (0.0-1.0)")
    pinsc.add_argument("--smooth-corners", action="store_true", help="Smooth sharp corners")
    pinsc.set_defaults(func=command_inkscape)
