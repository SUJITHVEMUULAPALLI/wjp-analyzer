...
# Import visual dashboard components
try:
    from wjp_analyser.web.components.visual_dashboard import (
        ...
        render_layered_nesting_preview,
        render_optimization_recommendations,
        render_waterjet_compliance_checklist,
        render_technical_summary_table,
        render_export_buttons,
    )
    VISUAL_DASHBOARD_AVAILABLE = True
except ImportError:
    VISUAL_DASHBOARD_AVAILABLE = False
    st.warning("Visual dashboard components not available. Using basic display.")

# Import professional report components
try:
    from wjp_analyser.web.components.professional_report import (
    ...
    st.warning("Professional report components not available.")

# Import PDF export components
try:
    from wjp_analyser.web.components.simple_pdf_export import (
        export_pdf_from_html_button,
        export_html_report_button,
        show_report_preview,
    )
    PDF_EXPORT_AVAILABLE = True
except ImportError:
    PDF_EXPORT_AVAILABLE = False
    st.warning("PDF export components not available.")

# Import interactive object management components
try:
    from wjp_analyser.web.components.interactive_objects import (
        render_object_selection_panel,
        render_group_selection_panel,
        render_edit_options_panel,
        render_object_group_summary,
    )
    INTERACTIVE_OBJECTS_AVAILABLE = True
except ImportError:
    INTERACTIVE_OBJECTS_AVAILABLE = False
    st.warning("Interactive object management not available.")

# Import interactive SVG viewer components
try:
    from wjp_analyser.web.components.dxf_to_svg import (
        convert_dxf_to_interactive_svg,
        create_simple_interactive_svg,
        get_object_info,
    )
    from wjp_analyser.web.components.interactive_svg_viewer import (
        interactive_svg_viewer,
        create_object_selection_panel,
        create_layer_filter_panel,
        create_interactive_preview_controls,
    )
    INTERACTIVE_SVG_AVAILABLE = True
except ImportError:
    INTERACTIVE_SVG_AVAILABLE = False
    st.warning("Interactive SVG viewer not available.")
...
if VISUAL_DASHBOARD_AVAILABLE:
    add_custom_css()

# Add professional report styling
if PROFESSIONAL_REPORT_AVAILABLE:
    add_report_styling()

# Display mode toggle - Simplified to essential modes
display_mode = st.sidebar.selectbox(
    "Display Mode",
    ["Interactive Preview", "Professional Report", "Classic View"],
    index=0,
    help="Choose between interactive preview (default), professional report, or classic view"
)
...
if display_mode == "Interactive Preview":
    ...
    ">
        ...
        <p style="margin: 10px 0 0 0; font-size: 1.1rem; opacity: 0.9;">
            Click objects to select • Layer-based coloring • Grid background • Enhanced visualization
        </p>
    </div>
    """, unsafe_allow_html=True)
elif display_mode == "Professional Report":
    st.markdown("""
    <div style="text-align: center; margin-bottom: 2rem;">
        ...
        <p style="color: #6b7280; font-size: 1.2rem; margin: 0.5rem 0;">
            Structured, one-page professional DXF analysis report
        </p>
    </div>
    """, unsafe_allow_html=True)
else:
...
if hasattr(st, 'session_state') and st.session_state and "_wjp_analysis_report" in st.session_state:
    ...
    # Results sections - Single unified section to avoid duplicates
    if display_mode == "Interactive Preview":
        # Interactive Preview Mode - Bidirectional object selection with summary
        st.markdown("## 📊 Analysis Summary")
        
        # Calculate and display summary metrics
        components = active_report.get("components", [])
        groups = active_report.get("groups", {})
        ...
        cutting_length = round(total_perimeter / 1000, 1)
        cutting_cost = round(cutting_length * 800, 0)
        garnet_use = round(cutting_length * 0.6, 1)
        cutting_time = round(cutting_length * 0.03, 1)
        
        # Summary cards
        summary_data = [
            ("Objects", str(total_objects), "#22C55E", "🧩"),
            ("Groups", str(total_groups), "#3B82F6", "🗂️"),
            ("Cut Length", f"{cutting_length} m", "#FACC15", "✂️"),
            ("Cut Cost", f"₹ {cutting_cost:,.0f}", "#FB923C", "💰"),
            ("Time", f"{cutting_time} h", "#10B981", "⏱️"),
            ("Garnet", f"{garnet_use} kg", "#E879F9", "🧱"),
        ]
        ...
        for col, (label, value, color, emoji) in zip(cols, summary_data):
            ...
            " onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='translateY(0)'">
                ...
                <div style="font-size: 1.4rem; margin-top: 5px;">{value}</div>
            </div>
            """, unsafe_allow_html=True)
        
        st.markdown("## 🎯 Interactive DXF Preview")
        
        # Initialize session state for interactive selection
        if "_wjp_interactive_selected_object" not in st.session_state:
            st.session_state["_wjp_interactive_selected_object"] = None
        if "_wjp_interactive_object_mapping" not in st.session_state:
            st.session_state["_wjp_interactive_object_mapping"] = {}
        
        # Get components and create interactive SVG (already calculated above)
        layers = active_report.get("layers", {})
        
        if components:
            # Create interactive SVG using enhanced fallback method
            try:
                # Use enhanced fallback SVG creation as primary method
                from wjp_analyser.web.components.dxf_to_svg import create_fallback_svg
                svg_code = create_fallback_svg(components, width=800, height=600)
                ...
                st.success("✅ Interactive SVG created successfully!")
                
                st.session_state["_wjp_interactive_object_mapping"] = object_mapping
                ...
                with col1:
                    ...
                    clicked_object = interactive_svg_viewer(
                        svg_code,
                        object_mapping,
                        st.session_state["_wjp_interactive_selected_object"],
                        width=800,
                        height=600
                    )
                    
                    # Update selected object if clicked
                    if clicked_object and clicked_object != st.session_state["_wjp_interactive_selected_object"]:
                        st.session_state["_wjp_interactive_selected_object"] = clicked_object
                        st.rerun()
                ...
                with col2:
                    ...
                    selected_object, updated_mapping = create_object_selection_panel(
                        components,
                        st.session_state["_wjp_interactive_selected_object"],
                        object_mapping
                    )
                    
                    # Update selection if changed from dropdown
                    if selected_object != st.session_state["_wjp_interactive_selected_object"]:
                        st.session_state["_wjp_interactive_selected_object"] = selected_object
                        st.rerun()
                    
                    # Layer filter panel
                    st.markdown("---")
                    selected_layers = create_layer_filter_panel(layers)
                    
                    # Preview controls
                    st.markdown("---")
                    controls = create_interactive_preview_controls()
                
                # Display selected object details
                if st.session_state["_wjp_interactive_selected_object"]:
                    selected_obj_id = st.session_state["_wjp_interactive_selected_object"]
                    obj_info = get_object_info(selected_obj_id, object_mapping)
                    
                    st.markdown("### 📊 Selected Object Details")
                    
                    col1, col2, col3, col4 = st.columns(4)
                    
                    with col1:
                        st.metric("Object ID", selected_obj_id)
                ...
                st.error(f"Interactive SVG creation failed: {str(e)}")
                st.info("Displaying standard preview...")
                plot_components(active_report)
        else:
            st.warning("No components found for interactive preview.")
        ...
        with col1:
            if PDF_EXPORT_AVAILABLE:
                file_name = os.path.basename(str(work.get("dxf_path", "analyze_dxf")))
                export_pdf_from_html_button(active_report, file_name)
        with col2:
            if PDF_EXPORT_AVAILABLE:
                export_html_report_button(active_report, file_name)
        with col3:
            if st.button("🔄 Regenerate", use_container_width=True):
                st.rerun()
        with col4:
            if st.button("📊 Switch Mode", use_container_width=True):
                st.session_state["display_mode"] = "Professional Report"
                st.rerun()
        
    elif display_mode == "Professional Report" and PROFESSIONAL_REPORT_AVAILABLE:
        # Generate professional report
        file_name = os.path.basename(str(work.get("dxf_path", "analyze_dxf")))
        generate_professional_report(active_report, file_name)
        
        # Add interactive object management
        if INTERACTIVE_OBJECTS_AVAILABLE:
            st.markdown("---")
            
            # Initialize session state for selections
            if "_wjp_selected_objects" not in st.session_state:
                st.session_state["_wjp_selected_objects"] = []
            if "_wjp_selected_groups" not in st.session_state:
                st.session_state["_wjp_selected_groups"] = []
            
            # Create tabs for different management views
            tab1, tab2, tab3, tab4 = st.tabs(["🔍 Objects", "🗂️ Groups", "⚙️ Edit Options", "📊 Summary"])
            
            with tab1:
                selected_objects = render_object_selection_panel(
                    active_report.get("components", []),
                    st.session_state["_wjp_selected_objects"]
                )
                st.session_state["_wjp_selected_objects"] = selected_objects
            
            with tab2:
                selected_groups = render_group_selection_panel(
                    active_report.get("groups", {}),
                    active_report.get("components", []),
                    st.session_state["_wjp_selected_groups"]
                )
                st.session_state["_wjp_selected_groups"] = selected_groups
        ...
        with col4:
            if st.button("📊 Switch Mode", use_container_width=True):
                st.session_state["display_mode"] = "Interactive Preview"
                st.rerun()
        
    elif display_mode == "Classic View":
        # Classic View - Simple text-based interface
        st.markdown("## 📈 Analysis Results")
