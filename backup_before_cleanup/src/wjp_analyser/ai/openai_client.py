...
def ai_edge_stencil(input_path: str) -> Dict[str, Any]:
    """
    Produce a high-contrast black/white stencil using OpenAI Images API.
    ...
    try:
        ...
        prompt = (
            "Convert this image into a clear black and white stencil with only "
            "sharp outlines and solid fills suitable for vector tracing."
        )

        with tempfile.NamedTemporaryFile(suffix="_ai.png", delete=False) as tmp:
        ...
        try:
            ...
            r = requests.post("https://api.openai.com/v1/images/edits", json=payload, headers=headers, timeout=120)
            r.raise_for_status()
        ...
        return {"success": True, "output_path": processed_path}
