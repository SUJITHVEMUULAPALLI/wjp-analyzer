...
class FileBasedCache:
    ...
    def _resolve_cache_dir(self, desired: str) -> Path:
        """Choose a user-writable cache directory, falling back if needed."""
        p = Path(desired)
        try:
            ...
            fallback = Path(base) / "WJP_ANALYSER" / "cache"
            try:
                fallback.mkdir(parents=True, exist_ok=True)
                return fallback
            except Exception:
                # Final fallback: current working directory subfolder
                cwd_fallback = Path(os.getcwd()) / "cache"
                try:
